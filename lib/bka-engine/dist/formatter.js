...
exports.formatBKAStatement = formatBKAStatement;
exports.formatBKASummary = formatBKASummary;
exports.formatCostsByCategory = formatCostsByCategory;
 ...
 */
function formatBKAStatement(result, property, fullResult) {
    const lines = [];
    // Header
    ...
    if (fullResult.verzugszinsen && fullResult.verzugszinsen > 0.01) {
        ...
        lines.push(`anfallen (Basiszinssatz + 5% = ${formatPercentage(calculator_1.VERZUGSZINSEN_RATE)} p.a., 288 BGB).`);
 ...
 */
function formatBKASummary(fullResult) {
    const lines = [];
    lines.push('='.repeat(80));
 ...
 */
function formatCostsByCategory(fullResult) {
    const lines = [];
    ...
    for (const [category, amount] of Object.entries(costsByCategory)) {
        const label = types_1.BKACostCategoryLabels[category];
