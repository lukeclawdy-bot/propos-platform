...
exports.validateBKAInput = validateBKAInput;
exports.checkCostBalance = checkCostBalance;
 ...
 */
const ALWAYS_UMLAGEFÄHIG = [
    types_1.BKACostCategory.BETRIEBSSTROM,
    types_1.BKACostCategory.WASSERVERSORGUNG,
    types_1.BKACostCategory.ENTWAESSERUNG,
    types_1.BKACostCategory.HEIZUNG,
    types_1.BKACostCategory.WARMWASSER,
    types_1.BKACostCategory.AUFZUG,
    types_1.BKACostCategory.MUELLBeseITIGUNG,
    types_1.BKACostCategory.STRASSENREINIGUNG,
    types_1.BKACostCategory.GARTENPFLEGE,
    types_1.BKACostCategory.BELEUCHTUNG,
    types_1.BKACostCategory.SCHORNSTEINREINIGUNG,
    types_1.BKACostCategory.VERSICHERUNG,
    types_1.BKACostCategory.HAUSMEISTER
];
/**
 * Categories that MAY be umlagefähig depending on contract
 * These require explicit agreement in the lease
 */
const CONDITIONALLY_UMLAGEFÄHIG = [
    types_1.BKACostCategory.INSTANDHALTUNG, // Allgemeine Instandhaltung
    types_1.BKACostCategory.INSTANDSETZUNG, // Instandsetzung (Repairs)
    types_1.BKACostCategory.SCHOENHEITSREPARATUREN, // Schönheitsreparaturen
    types_1.BKACostCategory.TV_EMPFANG, // TV/Radio reception
    types_1.BKACostCategory.VERMIETUNGSKOSTEN // Kosten der Vermietung (commercial only)
];
/**
 ...
 */
const NIU_CATEGORIES = [
    types_1.BKACostCategory.INSTANDSETZUNG, // Instandsetzung
    types_1.BKACostCategory.SCHOENHEITSREPARATUREN // Schönheitsreparaturen
];
/**
 ...
 */
const COMMERCIAL_ONLY_CATEGORIES = [
    types_1.BKACostCategory.VERMIETUNGSKOSTEN
];
/**
 ...
 */
function isAllocationKeyAppropriate(category, key) {
    ...
    if (consumptionCategories.includes(category)) {
        ...
        if (key !== types_1.AllocationKey.VERBRAUCH) {
            return false;
        }
    }
    // Müllbeseitigung typically uses PERSONENZAHL
    if (category === types_1.BKACostCategory.MUELLBeseITIGUNG) {
        if (key !== types_1.AllocationKey.PERSONENZAHL && key !== types_1.AllocationKey.WOHNFLAECHE) {
            return false;
        }
    }
    return true;
 ...
 */
function getSuggestedKey(category) {
    return types_1.DefaultAllocationKeys[category];
}
/**
 ...
 */
function validateBKAInput(input) {
    const warnings = [];
    // Validate period
 ...
 */
function validateCostEntry(cost, input) {
    const warnings = [];
 ...
 */
function checkCostBalance(costs, vorauszahlungen) {
 ...
 */
function formatValidationResults(warnings) {
